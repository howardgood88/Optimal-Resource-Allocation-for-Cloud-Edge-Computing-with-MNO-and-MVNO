...
class UtilityFunc:
    ...
    class FTP:
        ...
        def bw_down(bw: float) -> float:
            bw = min(bw, ftp_bw_down_bmax)
            return max_score * math.log10(bw / 150 + 1) / math.log10(ftp_bw_down_bmax / 150 + 1)
...
class TaskDeployment:
    ...
    def deploy(self, candidate_vm_id: np.array, task: np.array, vm_list: dict) -> None:
        ...
        logging.info(f'task utilities: {utilities}\n')
        logging.info(f'task utility: {utility}\n')
    
        self.hour_utility[task_type_idx] += max(utility, -100)
        self.user_cost += cost
